...
{
    fprintf(stderr,
            "Usage:OMP_NUM_THREADS=<number of threads> %s [--data PATH] [--target COL] [--trees N] [--max-depth D] "
...
int main(int argc, char **argv)
{
    ...
    printf("train=%d test=%d trees=%d mtry=%d max_depth=%d min_samples=%d seed=%u threads=%d\n",
           train.n_samples, test.n_samples, forest.n_trees, forest.mtry,
           forest.max_depth, forest.min_samples_split, p.seed, omp_get_max_threads());
